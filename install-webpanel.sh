...
pip install flask flask-socketio eventlet pexpect
...
def run_script(option, extra_input=""):
    ...
    # اگر نیاز به ورودی اضافی باشد
    if extra_input:
        child.expect("\n")
        child.sendline(extra_input)

    # خواندن خروجی و ارسال به SocketIO
    while True:
        try:
            line = child.readline()
            if not line:
...
def send_input(data):
    pass  # در این نسخه نیازی نیست، pexpect تمام ورودی‌ها را مدیریت می‌کند
...
<style>
body { font-size: 18px; font-family: Tahoma, sans-serif; direction: rtl; }
...
<style>
body { font-size: 18px; font-family: Tahoma, sans-serif; direction: rtl; }
select, input, button { font-size: 16px; padding: 6px; margin: 5px; }
pre { background-color: #f0f0f0; padding: 10px; max-height: 400px; overflow: auto; }
</style>
</head>
<body>
...
<head>
<meta charset="UTF-8">
<title>خروجی اجرا</title>
<style>
body { font-size: 18px; font-family: Tahoma, sans-serif; direction: rtl; }
pre { background-color: #f0f0f0; padding: 10px; max-height: 500px; overflow: auto; }
