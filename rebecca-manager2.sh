...
update_rebecca() {
    confirm_and_run "update Rebecca" rebecca update
}

install_rebecca_node() {
    confirm_and_run "install Rebecca Node" sudo bash -c "$(curl -sL https://github.com/rebeccapanel/Rebecca-scripts/raw/master/rebecca-node.sh)" @ install
}
...
while true; do
    ...
    echo -e "${GREEN}19) Rebecca update${NC}"
    echo -e "${GREEN}20) Install Rebecca Node${NC}"
